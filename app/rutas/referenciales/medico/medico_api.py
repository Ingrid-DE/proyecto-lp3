...
def getMedico(medico_id):
    ...
    try:
        ...
        app.logger.error(f"Error al obtener medicos: {str(e)}")
        return jsonify({
            'success': False,
            'error': 'Ocurrió un error interno. Consulte con el administrador.'
        }), 500
...
def addMedico():
    ...
    try:
        ...
        if medico_id is not None:
            ...
            return jsonify({ 'success': False, 'error': 'No se pudo guardar la medico. Consulte con el administrador.' }), 500
    except Exception as e:
...
def updateMedico(medico_id):
    ...
    try:
        if meddao.updateMedico(medico_id, descripcion.upper()):
            return jsonify({
                'success': True,
                'data': {'id': medico_id, 'descripcion': descripcion},
                'error': None
...
def deleteMedico(medico_id):
    ...
    try:
        ...
        if meddao.deleteMedico(medico_id):
            ...
            return jsonify({
                'success': False,
                'error': 'No se encontró al medico con el ID proporcionado o no se pudo eliminar.'
            }), 404
